...
# search df
def df_search(df: pd.DataFrame, substring: str, case: bool = False) -> pd.DataFrame: 
...
if st.session_state.page == 0:
    ...
    df_prop = df_search(drugs_properties_df, drug_input)
    st.dataframe(df_prop)

    curated_solubility_dataset_filted = df_search(curated_solubility_dataset_df, drug_input)
    st.dataframe(curated_solubility_dataset_filted)

    solubility_data_filted = df_search(solubility_data_df, drug_input)
    st.dataframe(solubility_data_filted)

    solubility_delaney_processed_filted = df_search(solubility_delaney_processed_df, drug_input)
    st.dataframe(solubility_delaney_processed_filted)

    solubility_delaney_filted = df_search(solubility_delaney_df, drug_input)
    st.dataframe(solubility_delaney_filted)

    FDA_APPROVED_filted = df_search(FDA_APPROVED_df, drug_input)
    ...
    Drugs_FDA_2024_filted = df_search(Drugs_FDA_2024_df, drug_input)
    ...
    products_to_inactive_ingredients_filted = df_search(products_to_inactive_ingredients_df, drug_input)
    ...
    df_inactive_iigs = df_search(products_to_inactive_ingredients_df, drug_input)
